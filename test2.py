...
def getUrl(url,tagName, Attrs, reg):
    try:
        proxy = proxies.get()
        html = requests.get(url,headers=random.choice(Webheader),proxies=proxy).text
        soup = BeautifulSoup(html,"html.parser") 
        content = str(soup.find_all(name = tagName, attrs = Attrs))
        proxies.put(proxy)
        res = re.findall(reg ,content) 
        print("链接获取成功",url,"len",len(res),"len of html",len(html))
        return res
    except Exception as e:
        if(proxies.qsize() <= 2):
            print("getUrl()链接获取失败，代理用完，正在重新获取代理......")
            get_proxy()
        print("getUrl()链接获取失败，正在重新调用getUrl()......")
        print(e)
        return getUrl(url,tagName, Attrs, reg)
...
def get_proxy():
    ...
    while proxiesNum < 5 and not IPS.empty() and not PORTS.empty():
        proxy = {"http":IPS.get() + ":" + PORTS.get()}
        try:
            html = requests.get("http://www.baidu.com",headers=random.choice(Webheader),proxies=proxy,timeout=2)
            proxies.put(proxy)
            proxiesNum += 1
...
class getThirdImgUrls(threading.Thread):
    ...
    def run(self):
        global ThirdImgUrlNum
        print("获取ThirdUrls线程",self.ThreadID,"开始")
        while ThirdImgUrlNum > 0:
            if(not SecondimgUrls.empty()):
                Url = SecondimgUrls.get()
                print("SecondimgUrls",Url)
                getThirdImgUrlsWorker(Url,self.ThreadID)
                SecondimgUrls.task_done
            else:
                break
        print("获取ThirdUrls线程",self.ThreadID,"结束")
def getImgUrlsWorker(imgHtmlUrl,ThreadID):
    global ImgNum
    #//TODO 
    res = getUrl(imgHtmlUrl, "div", {"class":"album_wrap"}, RegSrc)
    print(imgHtmlUrl,"len",len(res))
    for url in res:
        #需要控制进入队列的个数，加锁
        with mutex:
            if 0 < ImgNum:
                imgUrls.put(url)
                print(ImgNum,imgHtmlUrl)
                ImgNum -= 1
    ...
    def run(self):
        ...
        while True:
            if(not ThirdImgUrls.empty()):
                ThirdUrl = ThirdImgUrls.get()
                print("ThirdImgUrl",ThirdUrl,self.ThreadID)
                getImgUrlsWorker(ThirdUrl,self.ThreadID)
            ...
            try:
                ...
                if res.status_code == 200:
                    with open(filename,'wb') as f:
                        print(url, "Downloading...... Done",self.ThreadID)
                        f.write(res.content)
                else:
                    print("failed with res.status_code",response.status_code)
            except Exception as e:
                imgUrls.put(url)
                if proxies.qsize() <= 2:
                    print("getImg()代理用完，正在重新获取代理")
                    get_proxy()
...
def getThirdUrl():
    threads = []
    for i in range(min(ThirdImgUrls.qsize(),ThreadsNum)):
        t = getThirdImgUrls(i)
        t.start()
        threads.append(t)
...
if __name__=="__main__":
    ...
    if "-h" in argvs:
        ...
        for url in getUrl(HomeUrl+"/mm",'div', {"class":"category_main"},RegHref):
            ...
            SecondimgUrls_First.put(HomeUrl + url)
        print("开始构造SecondUrls")
        for i in range(min(SecondimgUrls_First.qsize(),ThreadsNum)):
        ...
        print("开始下载图片")
        threads = []
        for i in range(min(imgUrls.qsize(),ThreadsNum)):
            t = getImg(i) 
            t.start()
            threads.append(t)
        for thread in threads:
